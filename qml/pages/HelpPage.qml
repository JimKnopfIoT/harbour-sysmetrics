...
Page {
    ...
    property var groups: [
        ...
        {
            title: qsTr("Binder"), key: "binder",
            items: [
                { t: "Binder", d: qsTr("The IPC the Android side of the system is built on — what D-Bus is to the Sailfish side. Same purpose, different build. D-Bus relays through a daemon in user space and copies every message twice; binder is a driver in the kernel. The caller makes one ioctl, the driver copies the message once, straight into a memory region the receiver mapped when it opened the device, and wakes one thread of its pool. That is why Android sends every camera frame and every sensor reading across it — and why the driver, sitting in the middle, can count every single call and hand the tally out for free. This page reads that tally and nothing else.") },
                { t: qsTr("What runs over it here"), d: qsTr("Two things that have nothing to do with each other. First the hardware: camera, GPS, sensors, radio, lights and bluetooth are Android services from the device maker, and Sailfish talks to them through the gbinder library — without binder a port has no camera. Second App Support, which brings binder domains of its own and carries the traffic of every Android app in them. Everything else on the phone — ofono, mce, the fingerprint daemon — runs on D-Bus and does not appear here at all.") },
                { t: qsTr("Domain (context)"), d: qsTr("One binder device with a registry of its own. Processes on different domains cannot reach each other. The usual three are binder (framework), vndbinder (vendor) and hwbinder (hardware layers); App Support installs its own. Which domain is whose is read from the configuration files in /etc/gbinder.conf and /etc/gbinder.d — never guessed from the name.") },
                { t: qsTr("Node"), d: qsTr("A service object inside the process that owns it. The number is handed out by the driver and holds only until the next boot. It is the honest key to who serves what: the kernel prints every node in the section of its owner, so a node that moved to another process cannot keep an old claim.") },
                { t: qsTr("Handle"), d: qsTr("The number by which a caller addresses somebody else's node. The same service has a different handle in every process that holds it — which is why a handle alone never names a service.") },
                { t: qsTr("Service manager (handle 0)"), d: qsTr("The phone book of a domain, always reachable at handle 0. A service registers its name there; a caller asks for a name and gets back a handle. It is the one address every process knows in advance, which is what makes the rest work. A dead reply on handle 0 therefore does not mean one service is missing — it means nothing is answering for the whole domain.") },
                { t: qsTr("Reference"), d: qsTr("A hold one process keeps on another process's node. The driver counts them and keeps the node alive as long as one is left. The nodes/references figure is that tally for the whole device: how many service objects exist, and how many holds there are on them.") },
                { t: qsTr("Transaction"), d: qsTr("One call, or its reply. Normally it is finished in microseconds. A call that is still in flight on the next reading is reported as not returning — the time given for it is the gap between the two readings and therefore a lower bound.") },
                { t: qsTr("One-way call"), d: qsTr("A call the sender does not wait for. The driver parks it in a fixed buffer belonging to the receiver; when that buffer fills, further one-way calls are refused. The remaining space is the kernel's own figure, the point at which this app calls it tight (64 KiB) is not.") },
                { t: qsTr("Thread pool"), d: qsTr("Every process that answers binder calls keeps a pool of threads for them and tells the driver how large it may grow. Threads at that maximum with none ready means the next caller waits. The threads are named binder:<pid>_<n> after the process that owns the pool — that pid is what turns a spinning nameless thread into a process with a name.") },
                { t: qsTr("Death notification"), d: qsTr("A standing request to the driver: tell me when the process behind this handle dies. It is how a client learns that the service it holds is gone, instead of finding out at its next call. The counter says how many such requests are in force right now.") },
                { t: "BR_DEAD_REPLY", d: qsTr("The driver's answer when the target was gone or the handle was invalid: the call never reached a service. One of these now and then is ordinary; a steady rate of them is a caller talking to something that no longer exists.") },
                { t: "BR_FAILED_REPLY", d: qsTr("The driver refused the transaction before delivering it — a bad reference in the payload, a size the receiver would not take, a security check. The negative number beside it is the errno (-22 = EINVAL).") },
                { t: "BR_FROZEN_REPLY", d: qsTr("The target process is frozen, so the call was not taken. Expected while an App Support container sleeps.") },
                { t: qsTr("Transaction ring"), d: qsTr("The driver remembers the last 32 transactions and the last 32 failures, and nothing older. Every rate taken from it is therefore a lower bound, and this app says so wherever it prints one.") },
                { t: qsTr("Ping"), d: qsTr("The liveness check every binder service must answer. It runs no code of the service beyond the answer, changes nothing, and is what makes it possible to say which process serves a name: the kernel logs the transaction, and the log entry — not the name — is the proof.") },
                { t: qsTr("Orphaned node"), d: qsTr("A node whose owning process has ended while another process still holds a reference to it. The next call on that reference gets a dead reply. Read on request only: it comes out of the driver state dump, which prints every node of every process and takes the driver locks while doing it.") },
                { t: qsTr("Where the figures come from"), d: qsTr("/dev/binderfs/binder_logs on a current port, where the files are world readable and no privilege is needed; /sys/kernel/debug/binder on older ones, where they usually belong to root and the helper reads them. Both carry the same figures, printed differently — a kernel from before binderfs names no domain and counts no pages.") }
            ]
        },
        {
